...
| REQ-MP-08 | El módulo principal cuando reporta un error además del número de línea y la razón del error, muestra la línea en cuestión y apunta al lugar donde se produjo. Por ejemplo para el siguiente programa “hola” v1 \+ \# chau if class} Error Léxico en línea 2: \# no es un símbolo válido Detalle: v1 \+ \# chau ^ \[Error:\#|2\] |
...
| REA-AL-04 | El analizador debe reconocer al identificador de clase o interfaz que es una letra mayúscula seguida de uno o más caracteres que puede ser **letras, mayúsculas, minúsculas, dígitos o underscores.** Estos son denotados como **idClase.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| REQ-AL-05 | El analizador debe reconocer identificador de parámetroque es una única letra mayúscula. Estos son denotados como **idGen.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| REQ-AL-19 | Cualquier otra entrada que no conforme con las reglas presetnada debe generar error.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
`pr_if` — `if`
Notas: probado explícitamente el límite con `idMV`: `if0` no matchea esta keyword, queda como `idMV(if0)` (ver Casos de prueba).
...
### Literales de carácter y string (REQ-AL-10/11/20)

`charLiteral` — `'([^\\'\n\r]|\\u[0-9a-fA-F]{4}|\\[^\n\r])'`
Notas: contenido de un solo carácter cualquiera salvo `\`, `'` o un salto de línea real, **o** un escape. El escape es `\` + `u` + exactamente 4 hex (Unicode, REQ-AL-20), **o** `\` + cualquier otro carácter salvo un salto de línea real (por eso `\z`, `\'`, `\\`, `\ ` son todos válidos: no exige que sea un escape "reconocido" como en Java). Si después de `\u` no vienen 4 hex, si el contenido queda vacío (`''`), o si sobra más de un carácter antes de la comilla de cierre, no matchea como `charLiteral` — son errores (ver Casos de prueba para el detalle de las cascadas que puede disparar cada uno).

`litString` — `"([^"\\\n\r]|\\u[0-9a-fA-F]{4}|\\[^\n\r])*"`
Notas: igual que `charLiteral` pero con **cero o más** caracteres/escapes en vez de exactamente uno, y `"` en vez de `'` como delimitador (por eso una comilla doble escapada `\"` no cierra el string, REQ-AL-11). Un `\` justo antes de un salto de línea real **no** es un escape válido — corta el string igual que si no hubiera backslash (ver "Correcciones aplicadas" en Casos de prueba).

### Símbolos de puntuación (REQ-AL-14)

`parA` — `(`
Notas: literal, sin ambigüedad con ningún otro token.
...
`punto` — `.`
Notas: siempre este token — nunca es el inicio de un float, porque REQ-AL-21 exige que el float empiece con dígito. Por eso `.5` son dos tokens (`punto` + `intLiteral`), no uno.
...
`op<` — `<`
Notas: válido solo, sin necesitar un segundo carácter. Si lo sigue `=`, gana `op<=`.
...
`op==` — `==`
Notas: se resuelve por maximal munch sobre `op=`: si el segundo `=` no acompaña, queda en `op=` y el carácter siguiente se relee normal.
...
`op&&` — `&&`
Notas: **a diferencia de los demás operadores de dos caracteres, acá no hay una versión de un solo carácter válida.** Un `&` suelto no es ningún token de miniJava (no está en la lista de REQ-AL-15) — si no lo sigue un segundo `&`, es error léxico en vez de quedar como un token de un carácter.
...
### Nota pendiente sobre el diagrama

La flecha directa `Modulo Principal → Fuente MiniJava` salta al Controller y al Model. Si es literal, rompe la separación de capas (la Vista tocando un recurso del Modelo); si es sólo conceptual (representar que el usuario aporta el path por línea de comandos), conviene aclararlo explícitamente para no generar ambigüedad de diseño. Pendiente de definir antes de la implementación.
...
El analizadorHandler juega el rokl de controller, entre el Módulo principal, el manejador de archivos y analizador léxico  
...
public interface AnalizadorLexico{  
	...
	public void startAnalizar()  
}

### Class Token

Clase token que representa el token identificado  
Debe tener enumerados los tokens identificados en los requerimientos de analizador léxico.
...
| Espacio sin escapar como contenido | `' '` | `charLiteral(' ')` | REQ-AL-10 excluye solo `\` y `'` del contenido sin escapar, no el espacio; coincide con Java, donde `' '` también es un char literal válido |
...
- **Caret en `estadoChar`** (rama "falta la comilla simple de cierre"): mismo desfasaje que ya se había corregido en `estadoEntero`/`estadoAnd`/`estadoOr` — `reportarError` se llamaba después de haber mirado un carácter de más. Con `'ab'`, por ejemplo, el `^` apuntaba a la `b` en vez de a la `a` (el último carácter real del lexema reportado, `'a`). Se corrigió pasando `nroColumna - 1`, igual que en los demás casos ya arreglados.
